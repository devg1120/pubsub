...
import {notObject} from './general.js';

function getSerializeValueWithClassName(target, options) {
  ...
  if (!_shouldIgnoreEnumerableProperties(target)) {
    if (options && options.ignoreProperties) {
      options.ignoreProperties.forEach((ignored) => {
        delete target[ignored];
      });
    }

    if (options && options.interceptProperties) {
...
function appendClassInfoAndAssignDataForBuiltinType(target, serializedObj) {
  let className = target.__proto__.constructor.name;
  if (className === 'Object') {
  ...
  if (className !== 'Object') {
    ...
    if (className === BUILTIN_CLASS_ARRAYBUFFER || className === BUILTIN_CLASS_SHAREDARRAYBUFFER) {
      ...
      serializedObj[BOOLEAN_FIELD] = target.valueOf();
      ...
      serializedObj[TIMESTAMP_FIELD] = target.getTime();
...
function _shouldIgnoreEnumerableProperties(target) {
  const className = target.__proto__.constructor.name;
