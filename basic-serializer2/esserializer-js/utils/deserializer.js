...
function deserializeFromParsedObj(parsedObj, classes, options)  {
  return deserializeFromParsedObjWithClassMapping(parsedObj, getClassMappingFromClassArray(classes), options);
}

function deserializeFromParsedObjWithClassMapping(parsedObj, classMapping, options) {
  ...
  const classNameInParsedObj = parsedObj[CLASS_NAME_FIELD];
  ...
  if (options && options.fieldsForConstructorParameters) {
  ...
  const deserializedObj = deserializeClassProperty(classMapping[classNameInParsedObj], constructorParameters);
  return deserializeValuesWithClassMapping(deserializedObj, parsedObj, classMapping, options);
}

function _deserializeArray(parsedObj, classMapping) {
...
function deserializeClassProperty(classObj, constructorParameters) {
  if (!classObj) {
    return {};
  }
...
function deserializeValuesWithClassMapping(deserializedObj, parsedObj, classMapping, options) {
  for (const k in parsedObj) {
    const v = parsedObj[k];

    if (options && options.ignoreProperties && options.ignoreProperties.includes(k)) {
      continue;
    }
    if (options && options.rawProperties && options.rawProperties.includes(k)) {
